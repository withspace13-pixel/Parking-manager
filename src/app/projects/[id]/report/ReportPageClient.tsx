        ...
        <div className="mx-auto max-w-6xl px-6 py-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-semibold text-[var(--text)]">보고서</h1>
            </div>
            <button
              type="button"
        ...
        <div className="mt-8 card-raise overflow-hidden p-0">
          ...
          <ul className="divide-y divide-[var(--border)]">
            {dayFreeList.length === 0 ? (
              ...
              dayFreeList.map((row) => (
                <li
                  key={`${row.date}-${row.vehicle_num}`}
                  className="flex flex-wrap items-baseline gap-x-3 gap-y-2 px-6 py-5 text-sm transition-colors table-row-hover"
                >
